...
weights=ROOT / 'x_kidney_stone.pt'  # 权重文件地址   .pt文件
source=ROOT / 'data/results'  # 测试数据文件(图片或视频)的保存路径
yaml=ROOT / 'data/myvoc.yaml'  # 标签文件地址   .yaml文件
...
# device='cpu'  #modified by flower on 230306  as AssertionError: Invalid CUDA '--device 0' requested, use '--device cpu' or pass valid CUDA device(s)
